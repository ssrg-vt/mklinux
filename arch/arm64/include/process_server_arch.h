...
/* Inline functions for PTE flags manipulation */

static inline pte_t pte_clear_valid_entry_flag(pte_t pte)
{
	return pte_val(pte) & ~PTE_VALID;
}

static inline pte_t pte_set_valid_entry_flag(pte_t pte)
{
	return pte_val(pte) | PTE_VALID;
}

static inline pte_t pte_clear_user_access_flag(pte_t pte)
{
	return pte_val(pte) & ~PTE_USER;
}

static inline pte_t pte_set_user_access_flag(pte_t pte)
{
	return pte_val(pte) | PTE_USER;
