 ...
 * Initial Test version DO NOT USE
 * B M Saif Ansary<bmsaif86vt.edu> 2014
 * 
 */
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Michael Drescher <mdresch@vt.edu>");
MODULE_DESCRIPTION("Driver for Popcorn Messaging layer over Intel SCIF");
MODULE_SUPPORTED_DEVICE("mic/scif");
...
static void __exit unload(){
	...
	printk(KERN_INFO "Popcorn SCIF Messaging Layer Unloaded\n");
}
...
while(TRUE){		
		...
		if((no_bytes = scif_recv(newepd, curr_addr, dflt_size, SCIF_RECV_BLOCK))>=0)
		{
			if(no_bytes<0)
				continue;
