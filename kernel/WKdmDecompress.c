...
static WK_word*
WK_unpack_3_twentybits(WK_word *input_buf,
                    WK_word *input_end,
                    WK_word *output_buf) {
  ...
  while (input_next < input_end) {
    ...
    output_next[0] = temp & packing_mask;
    output_next[1] = (temp >> 20) & packing_mask;
    output_next[2] = (temp >> 40) & packing_mask;;
...
void
WKdm_decompress_and_diff  (WK_word* src_buf,
		WK_word* dest_buf)
{
 ...
 //                     LOW_BITS_AREA_END(src_buf),
  //                    tempLowBitsArray);
                      ...
                      WK_unpack_3_twentybits(LOW_BITS_AREA_START(src_buf),
                      LOW_BITS_AREA_END(src_buf),
                      tempLowBitsArray);

#ifdef WK_DEBUG
  printk("AFTER UNPACKING, about to enter main block \n");
#endif
...
#ifdef WK_DEBUG
    printk("next_output is %u\n", next_output);

    printk("next_tag is %u \n", next_tag);
    printk("tags_area_end is %u\n", tags_area_end);
    printk("next_q_pos is %u\n", next_q_pos);
    printk("next_low_bits is %u\n", next_low_bits);
    printk("next_full_word is %u\n", next_full_word);
#endif
    ...
    while (next_tag < tags_area_end) {
       ...
       switch(tag) {
         ...
         case EXACT_TAG: {
            ...
            *next_output = (*next_output)^(*dict_location);
            break;
         }
         case PARTIAL_TAG: {
            WK_word *dict_location = dictionary + *(next_q_pos++);
            {
               ...
               *next_output = (*next_output)^temp;    /* and echo it to output */
            }
            break;
         }
         case MISS_TAG: {
            ...
            *next_output = (*next_output)^missed_word;
            break;
         }
       }
       next_tag++;
