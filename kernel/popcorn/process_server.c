	...
	if (mm_data->alive == 0  && atomic_read(&(mm_data->pending_migration))==0) {
		...
		if (flush == 1 && mm_data->arrived_op == 0) {
			...
			if(atomic_read(&(mm_data->pending_migration))!=0)
				printk(KERN_ALERT"ERROR pending migration when cleaning memory\n");
		...
		if (is_last_thread_in_local_group) {
			...
			if (count == 0) {
				...
				if (flush == 0) {
					...
					printk("ERROR: flush is 1 during first exit (alive set to 0 now)\n");
					return 1;
				}
			}else {
				/*
...
start: if (pte == NULL || pte_none(value_pte)) {
		...
		*/
	       ...
	       ret = do_remote_fetch_for_2_kernels(tsk->tgroup_home_cpu, tsk->tgroup_home_id, mm,
						   vma, address, page_fault_flags, pmd, pte, value_pte, ptl, _cpu);
			...
			*/
				...
				*/
					...
					*/
				       ...
				       ret = do_remote_write_for_2_kernels(tgroup_home_cpu, tgroup_home_id, mm, vma,
									   address, page_fault_flags, pmd, pte, ptl, page,0, _cpu);
#else
				...
				* */
					...
					*/
				       ...
				       if (page_fault_flags & FAULT_FLAG_WRITE)
					       ret = do_remote_write_for_2_kernels(tgroup_home_cpu, tgroup_home_id, mm, vma,
										   address, page_fault_flags, pmd, pte, ptl, page,1, _cpu);
				       else
					       ret = do_remote_read_for_2_kernels(tgroup_home_cpu, tgroup_home_id, mm, vma,
										  address, page_fault_flags, pmd, pte, ptl, page, _cpu);
#else
...
out: down_write(&current->mm->mmap_sem);
     ...
     return start_distribute_operation(VMA_OP_UNMAP, start, len, 0, 0, 0, 0,
				       NULL, 0, _cpu, &request_distributed_vma_op);
#endif
#endif
}
...
#else
#if NOT_REPLICATED_VMA_MANAGEMENT
       end_distribute_operation(VMA_OP_UNMAP, start_ret, start, _cpu,
				&request_distributed_vma_op);
       return 0;
#endif
#endif
...
#else
#if NOT_REPLICATED_VMA_MANAGEMENT
       end_distribute_operation(VMA_OP_PROTECT, start_ret, start, _cpu,
				&request_distributed_vma_op);
       return 0;
#endif
#endif
...
#else
#if NOT_REPLICATED_VMA_MANAGEMENT
       end_distribute_operation(VMA_OP_MAP, start_ret, addr, _cpu,
				&request_distributed_vma_op);
       return 0;
#endif
#endif
...
long process_server_do_brk_start(unsigned long addr, unsigned long len) {
       ...
       return start_distribute_operation(VMA_OP_BRK, addr, len, 0, 0, 0, 0, NULL,
					 0, _cpu, &request_distributed_vma_op);
#endif
#endif
...
#else
#if NOT_REPLICATED_VMA_MANAGEMENT
       end_distribute_operation(VMA_OP_BRK, start_ret, addr, _cpu,
				&request_distributed_vma_op);
       return 0;
#endif
#endif
...
long process_server_do_mremap_start(unsigned long addr, unsigned long old_len,
       ...
       return start_distribute_operation(VMA_OP_REMAP, addr, (size_t) old_len, 0,
					 new_addr, new_len, flags, NULL, 0, _cpu,
					 &request_distributed_vma_op);
#endif
#endif
...
#else
#if NOT_REPLICATED_VMA_MANAGEMENT
       end_distribute_operation(VMA_OP_REMAP, start_ret, new_addr, _cpu,
				&request_distributed_vma_op);
       return 0;
#endif
#endif
