...
/* read optimized */
#define vfs_opt_read_enabled(x) ((vfs_optimization & VFS_OPT_READ) &&  \
		!bdi_cap_writeback_dirty(x) && !kernel_client)
/* write optimized */
#define vfs_opt_write_enabled(x) ((vfs_optimization & VFS_OPT_WRITE) && \
		!bdi_cap_writeback_dirty(x) && !kernel_client)
