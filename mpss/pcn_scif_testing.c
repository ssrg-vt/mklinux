...
*/	
		...
		if(wait_data->dst_cpu==my_cpu)
		{
			...
			exec_data->msg=kmalloc(lmsg->hdr.size,GFP_KERNEL);
...
//	pcn_kmsg_cbftn* cbtbl = (pcn_kmsg_cbftn*)arg0;
	...
	//	dma_rcv_buffer=kmalloc(2*PAGE_SIZE, GFP_KERNEL);
		...
		BARRIER(newepd,"Register RCV");
			msg = (struct pcn_kmsg_message*)kmalloc(sizeof(struct pcn_kmsg_long_message),GFP_KERNEL);
		up(&rcv_connDone);
...
while(TRUE){		
	...
	//	msg = (struct pcn_kmsg_message*)kmalloc(2*PAGE_SIZE,GFP_KERNEL);
