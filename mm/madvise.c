		...
		unsigned long start, unsigned long end, int behavior)
{
	...
	case MADV_REMOVE:
		if (current->tgroup_distributed==1 && current->distributed_exit==EXIT_ALIVE) {
			distributed =1;
			_ret = vma_server_madvise_remove_start(vma->vm_mm, start, (end-start));
		...
		if (current->tgroup_distributed==1 && distributed==1)
			vma_server_madvise_remove_end(vma->vm_mm, start, (end-start), _ret);
		return ret;
	case MADV_WILLNEED:
	...
	case MADV_DONTNEED:
		if (current->tgroup_distributed==1 && current->distributed_exit==EXIT_ALIVE) {
			distributed =1;
			_ret = vma_server_do_unmap_start(vma->vm_mm, start, (end-start));
			if (_ret<0 && _ret!=VMA_OP_SAVE && _ret!=VMA_OP_NOT_SAVE)
				return _ret;
		}
		ret = madvise_dontneed(vma, prev, start, end);
		if (current->tgroup_distributed==1 && distributed==1)
			vma_server_do_unmap_end(vma->vm_mm, start, (end-start), _ret);
		return ret;
	default:
