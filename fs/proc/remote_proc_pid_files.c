...
static DEFINE_MUTEX(remote_cpuset_mutex);
...
int fill_response(struct task_struct *task, _remote_pid_stat_response_t *res)
{
	...
	res->_cutime = res->_cstime = res->_utime = res->_stime = 0;
	res->_cgtime = res->_gtime = 0;
	...
	if (lock_task_sighand(task, &flags)) {
		...
		if (sig->tty) {
			...
			if (whole) {
				...
				thread_group_cputime_adjusted(task, &res->_utime, &res->_stime);
				res->_gtime = res->_gtime + sig->gtime;
			}
		...
		if (!whole) {
			...
			task_cputime_adjusted(task, &res->_utime, &res->_stime);
...
int fill_cpuset_response(struct pid *pidp, _remote_pid_cpuset_response_t *res)
{
		...
		//Sharath: Ported to Linux 3.12
		//cgroup_lock();
		mutex_lock(&remote_cpuset_mutex);
		//Sharath: Ported to Linux 3.12
