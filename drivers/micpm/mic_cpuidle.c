...
/* FROM MPSS3.2 --- have a look at arch/x86/kernel/apic/apic.c: enable_IR_x2apic(void)
...
//			     struct cpuidle_state *state)
{
	...
	idle_count[cpu]++;
	idle_time = __mic_idle_enter_c1(dev,
					dev->kobjs[index]->state);
	local_irq_enable();
	return idle_time;
}
...
//			      struct cpuidle_state *state)
{
	...
	if (!need_resched()) {
		idle_count[cpu]++;
		kt1 = ktime_get_real();
		__mic_idle_enter_c6(dev, dev->kobjs[index]->state, 0);
...
//			      struct cpuidle_state *state)
{
	...
	if(corec6_allowed(cpu)) {
		retval = __mic_idle_enter_c6(dev, dev->kobjs[index]->state, flush);
	} else {
		/* Save cpu state       */
		if(pc6_through_pc3()) {
			mic_save_processor_state(apicid);
			...
			__mic_idle_enter_c1(dev, dev->kobjs[index]->state);
			retval = MIC_STATE_C1;
		}	
	}
	smp_mb();
...
static int mic_setup_cpuidle(struct cpuidle_device *dev, int cpu)
{
	...
	//dev->state_count = count; //AB MPSS3.2 original
	...
	for_each_online_cpu(cpu) {
		...
		if (cpuidle_register_device(cidle)) {
			printk(KERN_DEBUG PREFIX
			       "Failed to register cpuidle device cpu %d\n", cpu);
			goto fail;
		}
