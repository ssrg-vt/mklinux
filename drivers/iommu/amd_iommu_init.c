...
static int __init add_special_device(u8 type, u8 id, u16 devid, bool cmd_line)
{
	...
	list_for_each_entry(entry, list, list) {
		if (!(entry->id == id && entry->cmd_line))
			...
			printk(KERN_ALERT"AMD-Vi: Command-line override present for %s id %d - ignoring\n",
					...
					struct ivhd_header *h)
{
	...
	while (p < end) {
		...
		case IVHD_DEV_ALL:
			...
			printk(KERN_ALERT"  DEV_ALL\t\t\t first devid: %02x:%02x.%x"
...
static void print_iommu_info(void)
{
	...
	printk(KERN_ALERT"AMD-Vi: irq_remapping_enabled %d \n",irq_remapping_enabled);
...
int __init amd_iommu_enable(void)
{
	...
	if (ret)
		return ret;
	printk(KERN_ALERT"THANK YOU IRQ U R ALIVE\n");
...
static int __init amd_iommu_init(void)
{
	...
	if (ret) {
		...
		printk(KERN_ALERT"secondary kernel \n");
