...
static u32 apm_xgenet_serdes_init(struct apm_enet_priv *priv, u32 rev)
{
	...
	if (!loop) {
		ENET_PRINT("INIT_SERDES : PLL is %sREADY\n", pll_ready ? "" : "not ");
		ENET_PRINT("INIT_SERDES : PLL %sLOCKed\n",
			pll_lock ? "" : "not ");
...
static int vco_status(struct apm_enet_priv *priv)
{
	...
	if (!vco_calibration) {
		ENET_PRINT("VCO_CALIB: PLL %sLOCKed\n", pll_lock ? "" : "not ");
		ENET_PRINT("VCO_CALIB: PLL VCO Calibration %s\n", vco_calibration ? "DONE" : "not done");
		ENET_PRINT("VCO_CALIB: PLL VCO Calibration %s : 0x%x\n", vco_cal_fail == 0 ? "NO FAILURE" : "FAILED",vco_cal_fail);
...
#ifndef __U_BOOT__
static int apm_xg_chip_version(void)
{
	...
	case 0x03: /* B0 */
		return 3;
	case 0x00: /* Not marked - Assume A3 */
	default:   /* Unknown */
		return 2;
	}
}
#else
int apm_xg_chip_version(void)
{
	char *revision = apm88xxx_chip_revision();

	if (!strncmp(revision, "A1", 2))
		return 0;
	else if (!strncmp(revision, "A2", 2))
		return 1;
	else if (!strncmp(revision, "A3", 2))
		return 2;
	else if (!strncmp(revision, "B0", 2))
		return 3;
	return 4;
}
#endif
