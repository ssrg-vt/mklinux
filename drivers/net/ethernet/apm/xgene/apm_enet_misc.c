...
int apm_enet_get_pb_cnt(struct apm_enet_priv *priv, int pbn)
{
	...
	ENET_DEBUG("%s: %d: reg_offset 0x%08x pbn_offset 0x%08x val 0x%08x\n",
...
void apm_enet_clr_pb(struct apm_enet_priv *priv, u8 qmtm_ip, u16 queue_id)
{
	...
	while (prefetched--)  {
		u32 data;
		apm_enet_qmi_pop_pb_msg(priv, qinfo.pbn, 3, 1, &data);
		data = apm_enet_get_pb_cnt(priv, qinfo.pbn - 0x20);
	}
