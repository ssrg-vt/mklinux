...
static void storm_qmtm_read_qstate(u8 qmtm_ip, u16 queue_id, void *qstate)
{
	...
	struct storm_qmtm_csr_qstate *qfabric =
		...
		(dev->qmtm_fabric_vaddr + (queue_id << 6));
	struct storm_qmtm_csr_qstate *csr_qstate =
		&((union storm_qmtm_qstate *)qstate)->csr;
...
static int storm_qmtm_set_qstate(struct xgene_qmtm_qinfo *qinfo)
{
	...
	QMTM_CORE_PRINT("QMTM_IP %d QID %d cfgacceptlerr %d cfgstartaddr 0x%llx cpu_notify %d\n",
		qinfo->qmtm_ip, qinfo->queue_id, pq_fp->cfgacceptlerr, cfgstartaddr, pq_fp->cpu_notify);
	QMTM_CORE_PRINT("QMTM_IP %d QID %d cfgnotifyqne %d cfgsaben %d cfgtmvq %d cfgtmvqen %d\n",
		qinfo->qmtm_ip, qinfo->queue_id, pq_fp->cfgnotifyqne, pq_fp->cfgsaben, pq_fp->cfgtmvq, pq_fp->cfgtmvqen);
