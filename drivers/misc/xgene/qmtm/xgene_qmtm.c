...
int xgene_qmtm_set_qinfo(struct xgene_qmtm_qinfo *set)
{
	...
	qinfo->qfabric = dev->qmtm_fabric_vaddr + (queue_id << 6);
	rc = pdev->set_qstate(qinfo);

	if (rc != QMTM_OK) {
		QMTM_PRINT("SETQ: Unable to configure qstate for %s "
	...
	qinfo->sab_command = qinfo->qfabric + 0x28;
		...
		u8 fq_pbn_start, u8 fq_pbn_count)
{
	...
	if (slave == SLAVE_MAX) {
		QMTM_PRINT("SETS: Slave %s doesn't exist\n", name);
		goto _ret_set_slave;
	}
	...
	if (pdev->sdev[slave]) {
		QMTM_PRINT("SETS: Slave %s already configured\n", name);
		goto _ret_set_slave;
	}
	...
	QMTM_CORE_PRINT("%s: Slave %d QMTM %d Slave ID %d WQ_PBN 0x%02x "
...
static int xgene_qmtm_enable(u32 qmtm_ip)
{
	...
	/* program threshold set 1 */
	xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD0_SET1_ADDR, 100);
	xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD1_SET1_ADDR, 200);
	
	/* program threshold set 6 */
        xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD0_SET6_ADDR, 1);
        xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD1_SET6_ADDR, 10);

        /* program threshold set 7 */
        xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD0_SET7_ADDR, 512);
        xgene_qmtm_wr32(qmtm_ip, CSR_THRESHOLD1_SET7_ADDR, 1024);
	
	/* program all hysteresis */
