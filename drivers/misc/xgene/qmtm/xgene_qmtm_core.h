...
/* SAB related structures */
typedef void (*qmtm_sab_fn_ptr) (void *data);

struct qmtm_sab_info {
        qmtm_sab_fn_ptr critical_entry;
        qmtm_sab_fn_ptr critical_exit;
        void *data;
};
...
struct xgene_qmtm_dev {
	...
	struct qmtm_sab_info qmtm_sab_info_table[QMTM_MAX_QUEUES]; /* SAB table */
	char sab_irq_s[16];
	char error_irq_s[16];
