...
#define  RXTX_REG158_SUM_CALIB_DONE_RD(src)	((0x00008000 & (u32)(src)) >> 15)
#define  RXTX_REG158_SUM_CALIB_ERR_RD(src)	((0x00004000 & (u32)(src)) >> 14)
#define  RXTX_REG158_LAT_CALIB_DONE_RD(src)	((0x00008000 & (u32)(src)) >> 11)
 ...
 * To enable, pass boot argument phy_xgene.preB0Chip=1
 */
static int preB0Chip;
MODULE_PARM_DESC(preB0Chip, "Enable pre-B0 chip support (1=enable 0=disable)");
module_param_named(preB0Chip, preB0Chip, int, 0444);
					...
					enum clk_type_t clk_type)
{
	...
	if (cmu_type == REF_CMU || !xgene_ahci_is_preB0()) 
...
static void xgene_phy_set_pq(struct xgene_phy_ctx *ctx, int lane, u32 param)
{
 	u32 val;	
	
	serdes_rd(ctx, lane, RXTX_REG125, &val);
	val = RXTX_REG125_SIGN_PQ_SET(val,param);
	if (param) 
	   val = RXTX_REG125_PQ_REG_SET(val,0x2);
   	else 
	   val = RXTX_REG125_PQ_REG_SET(val,
			ctx->sata_param.txeyetuning[lane * 3 +
			ctx->sata_param.speed[lane]]);
...
static int xgene_phy_set_mode(struct phy *phy, int lane, int mode, u32 param)
{
	struct xgene_phy_ctx *ctx = phy_get_drvdata(phy);
	...
	ctx->mode = mode;
	if (ctx->mode == MODE_SATA) {
		ctx->sata_param.disk_type[lane] = param ? DISK_HDD: DISK_SSD;
		if (!ctx->inited)
			return 0;
		switch(param) {
		case POST_LINK_UP_SSD :  		      
			return xgene_phy_set_ctle(ctx, lane,
				ctx->sata_param.txboostgain_ssd[lane * 3 +
				ctx->sata_param.speed[lane]]);
		case POST_LINK_UP_HDD :	  
		      return xgene_phy_set_ctle(ctx, lane,
				ctx->sata_param.txboostgain[lane * 3 +
				ctx->sata_param.speed[lane]]);
		case PRE_HARDRESET :     
		      xgene_phy_set_pq(ctx, lane, 1);
		      break;
		case POST_HARDRESET :     
		      xgene_phy_set_pq(ctx, lane, 0);
		      break;
	        case POST_LINK_UP_PHY_RESET :
		      xgene_phy_reset_rxd(ctx, lane);  
	              break;	      
		} 
	}
...
static int xgene_phy_set_speed(struct phy *phy, int lane, int mode, u32 speed)
{
	struct xgene_phy_ctx *ctx = phy_get_drvdata(phy);
	...
	if (lane >= ctx->lane)
		return -EINVAL;
	if (ctx->mode == MODE_SATA) {
		if ((speed - 1) == ctx->sata_param.speed[lane])	
			return 0;
		switch(speed){
		case 1:	
				   ...
				   struct of_phandle_args *args)
{
	...
	if (args->args_count > 1 && args->args[1] <= MAX_LANE) {
		ctx->lane = args->args[1];
		return ctx->phy;
	}
...
static void xgene_phy_get_param(struct platform_device *pdev, const char *name,
...
static const struct acpi_device_id xgene_phy_acpi_match[] = {
	{"APMC0D0A", XGENE_PHY_ACPI},
	{"APMC0D0B", XGENE_PHY_EXT_ACPI},
	{},
};
MODULE_DEVICE_TABLE(acpi, xgene_phy_acpi_match);

static int xgene_phy_acpi_match_id(struct platform_device *pdev)
{
	...
	for (i = 0; xgene_phy_acpi_match[i].id[0] != '\0'; i++)
		if (!strcmp(hid, xgene_phy_acpi_match[i].id))
			return xgene_phy_acpi_match[i].driver_data;
#endif
	return -ENODEV;
}

static struct phy_init_data *xgene_phy_load_consumer(struct xgene_phy_ctx *ctx)
{
#if defined(CONFIG_ACPI)
	...
	/* Consumer data is required for UEFI boot */
	if (efi_enabled(EFI_BOOT)) {
		struct acpi_dsm_entry entry;

		init_data = devm_kzalloc(ctx->dev, sizeof(*init_data),
					 GFP_KERNEL);
		if (!init_data)
			return NULL;
		consumers = devm_kzalloc(ctx->dev, sizeof(*consumers),
					 GFP_KERNEL);
		if (!consumers)
			return NULL;
		dev_name = devm_kzalloc(ctx->dev, 12, GFP_KERNEL);
		if (!dev_name)
			return NULL;
		init_data->num_consumers = 1;
		init_data->consumers = consumers;
		consumers->dev_name = dev_name;
		consumers->port = "sata-phy";

		if (acpi_dsm_lookup_value(ACPI_HANDLE(ctx->dev),
							"consumer", 0, &entry) || !entry.value)
			return NULL;
		strncpy(dev_name, entry.value, 11);
		dev_name[11] = '\0';
		kfree(entry.key);
		kfree(entry.value);
		return init_data;
...
static int xgene_phy_probe(struct platform_device *pdev)
{
	...
	ctx->dev = &pdev->dev;
	ctx->lane = 2;	/* default to 2 lanes */
	ctx->inited = UNINITIALIZED;
	...
	if (of_device_is_compatible(pdev->dev.of_node, XGENE_PHY_EXT_DTS) ||
	    (efi_enabled(EFI_BOOT) &&
	     xgene_phy_acpi_match_id(pdev) == XGENE_PHY_EXT_ACPI)) {
		res = platform_get_resource(pdev, IORESOURCE_MEM, res_idx++);
		if (!res) {
		...
		if (IS_ERR(ctx->ext_cmu_base)) {
			dev_err(&pdev->dev, "can't map external resource\n");
			rc = PTR_ERR(ctx->ext_cmu_base);
			goto error;
		}
	...
	xgene_phy_get_param(pdev, "apm,tx-boost-gain-ssd",
		ctx->sata_param.txboostgain_ssd, 6, default_txboost_gain_ssd,1);
	xgene_phy_get_param(pdev, "apm,tx-equalizer",
		ctx->sata_param.txequalizer, 6, default_dfe, 1);
	...
	for (i = 0; i < ctx->lane; i++) {
		...
		ctx->sata_param.disk_type[i] = DISK_HDD;/* Default to HDD */
	}
	...
	init_data = xgene_phy_load_consumer(ctx);
	ctx->phy = devm_phy_create(ctx->dev, &xgene_phy_ops, init_data);
	if (IS_ERR(ctx->phy)) {
...
static struct platform_driver xgene_phy_driver = {
	...
	.driver = {
		   ...
		   .acpi_match_table = ACPI_PTR(xgene_phy_acpi_match),
	},
};
