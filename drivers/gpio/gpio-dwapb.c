...
#include <linux/efi.h>
#define DEFAULT_NR_GPIO		32
#define DEFAULT_NR_PORTS	1
#define DEFAULT_PORT_IDX	0
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(3, 12, 0)
	ct->chip.irq_startup = dwapb_irq_reqres;
	ct->chip.irq_shutdown = dwapb_irq_relres;
#else
...
static int dwapb_gpio_add_port_acpi(struct platform_device *pdev, struct dwapb_gpio *gpio, unsigned int offs)
{
#ifdef CONFIG_ACPI
	struct dwapb_gpio_port *port;
	u64 port_idx = DEFAULT_PORT_IDX;
	...
	port->bgc.gc.ngpio = DEFAULT_NR_GPIO;

	/*
	 * Only port A can provide interrupts in all configurations of the IP.
	 */
	if (port_idx == 0 && INTERRUPT_ENABLED)
...
static int dwapb_gpio_probe(struct platform_device *pdev)
{
	...
	if (efi_enabled(EFI_BOOT))
		gpio->nr_ports = DEFAULT_NR_PORTS;
	else
#endif
...
#ifdef CONFIG_ACPI
	if (efi_enabled(EFI_BOOT))
		dwapb_gpio_add_port_acpi(pdev, gpio, offs);
	else
#endif
