...
#ifdef _trace
        #define p_trace_printk(...) trace_printk(__VA_ARGS__)
#else
        #define p_trace_printk(...)
#endif
...
//pcn_kmsg_send(i,
		...
		static void create_new_threads(thread_pull_t * my_thread_pull,
			...
			//printk(KERN_ALERT"%s: entered  count{%d}\n",__func__,count);
		...
		static int create_kernel_thread_for_distributed_process_from_user_one(
			...
			static int handle_mapping_response_void(struct pcn_kmsg_message* inc_msg) {
				...
				PSPRINTK("Answer_request_void %i address %lu from cpu %i. This is a void response. owner %d\n", answer_request_void, response->address, inc_msg->hdr.from_cpu,response->owner);
			...
			void process_mapping_request_for_2_kernels(struct work_struct* work) {
				...
				if (!vma || address >= vma->vm_end || address < vma->vm_start) {
					...
					if(_cpu == request->tgroup_home_cpu){
						printk("%s:OCCHIO vma NULL in xeon address{%lu} \n",__func__,address);
						up_read(&mm->mmap_sem);
						goto out;
					}
				} else {
				...
				if ((_cpu==request->tgroup_home_cpu) || memory->alive != 0) {
					...
					fetch:				PSPRINTK("concurrent request\n");
			 			...
			 			* */
						if (fetched_data->is_fetch != 1)
							printk("ERROR: find a mapping_answers_for_2_kernels_t not mapped and not fetch\n");
						...
						if(_cpu==request->tgroup_home_cpu){
							...
							PSPRINTK("%s: marking a pte for address %lu \n",__func__,address);
...
resolved:
				...
				// Send response
				pcn_kmsg_send_long(from_cpu, (struct pcn_kmsg_long_message*) (response),sizeof(data_response_for_2_kernels_t) - sizeof(struct pcn_kmsg_hdr) + response->data_size);
...
printk("void: response->vma_present %d response->vaddr_start %lu response->vaddr_size %lu response->prot %lu response->vm_flags %lu response->pgoff %lu response->path %s response->futex_owner %d\n",void_response->vma_present, void_response->vaddr_start , void_response->vaddr_size,void_response->prot, void_response->vm_flags , void_response->pgoff, void_response->path,void_response->futex_owner);
				...
				// Send response
				pcn_kmsg_send_long(from_cpu,(struct pcn_kmsg_long_message*) (void_response),sizeof(data_void_response_for_2_kernels_t) - sizeof(struct pcn_kmsg_hdr));
...
PSPRINTK("Handle request end\n");
 ...
 up_read(&mm->mmap_sem);
			...
			void synchronize_migrations(int tgroup_home_cpu,int tgroup_home_id ){
				...
				if(migrating_threads>0){
					
					while (atomic_read(&(memory->pending_back_migration))<migrating_threads) {
						msleep(1);
					}

					int app= atomic_add_return(-1,&(memory->pending_back_migration));
					if(app==migrating_threads)
				...
				//for synchronizing migratin threads
				 ...
				 if(memory){
					if(migrating_threads>0){
						int app= atomic_add_return(1,&(memory->pending_back_migration));
						if(app==migrating_threads)
							atomic_set(&(memory->pending_back_migration),(2*migrating_threads));
						}
					else
...
//static
 int do_remote_write_for_2_kernels(int tgroup_home_cpu, int tgroup_home_id,
	...
	//in this case I send and invalid message
		...
		PSPRINTK("Sending a write message for address %lu \n ", address);
	...
	int do_remote_fetch_for_2_kernels(int tgroup_home_cpu, int tgroup_home_id,
		...
		if(_cpu==tgroup_home_cpu){
			if(pte_none(value_pte)){
				...
				local_fetch++;
#endif
				PSPRINTK("Copy not present in the other kernel, local fetch %d of address %lu pid %d\n", local_fetch, address,current->pid);
		...
		PSPRINTK("Fetch %i address %lu pid %d\n", fetch, address,current->pid);
...
PSMINPRINTK(
	...
	if(page_fault_flags & FAULT_FLAG_WRITE){
		PSPRINTK("Page fault %i address %lu in page %lu task pid %d current pid %d {%d,%d} write \n", page_fault_mio, page_faul_address, address, tsk->pid,current->pid, tgroup_home_cpu, tgroup_home_id);

	}
	else{
	 	 PSPRINTK("Page fault %i address %lu in page %lu task pid %d current pid %d {%d,%d} read \n", page_fault_mio, page_faul_address, address, tsk->pid,current->pid, tgroup_home_cpu, tgroup_home_id);
	}
	...
	start: if (pte == NULL || pte_none(value_pte)) {
		...
		if (ret == -1 || ((page_fault_flags & FAULT_FLAG_WRITE) && ret == 0)) {
			...
			vma = find_vma(mm, address);
			if (unlikely(!vma || address >= vma->vm_end
					|| address < vma->vm_start)) {

				printk(	"ERROR: vma not valid after fetching it without errors\n");
				spin_unlock(ptl);
				return VM_FAULT_VMA;
			}
#if TIMING
	 ...
	 */
		 ...
		 */
		if (page->replicated == 0) {
			PSPRINTK("Page not replicated task pid %d current pid %d address %lu \n",tsk->pid,current->pid, address);
...
retry_cow:
		...
		if (page->status == REPLICATION_STATUS_VALID) {
			...
			PSPRINTK("Page status valid address %lu task pid %d current pid %d \n",address, tsk->pid,current->pid);
			 ...
			 * */
			if (page->status == REPLICATION_STATUS_WRITTEN) {
				PSPRINTK("Page status written task pid %d current pid %d address %lu \n",tsk->pid,current->pid, address);
				spin_unlock(ptl);
#if TIMING
				unsigned long long stop= native_read_tsc();
				unsigned long long time_elapsed= stop-start;
				...
				PSPRINTK("Page status invalid address %lu task pid %d current pid %d \n",address,tsk->pid,current->pid);
...
static int read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
	 ...
	 len=sprintf(page, "%s: migrating_threads: %d\n", __func__,migrating_threads);
...
static int write_proc (struct file *file, const char __user *buffer, unsigned long count, void *data)
{
  	int i;
        ...
        kstrtol_from_user(buffer, count, 0, &migrating_threads);
...
void process_vma_op(struct work_struct* work) {
	...
	PSVMAPRINTK("Received vma operation from cpu %d for tgroup_home_cpu %i tgroup_home_id %i operation %i my pid is %d\n", operation->header.from_cpu, operation->tgroup_home_cpu, operation->tgroup_home_id, operation->operation,current->pid);
...
//	printk("%s called\n", __func__);
	...
	if(migrating_threads>0){	 //to synchronize migrations...
		 while (atomic_read(&(memory->pending_migration))<migrating_threads) {
         		msleep(1);
          	}
...
static int clone_remote_thread(clone_request_t* clone,int inc) {
	...
	if (memory) {
		...
		if(inc){
			  	...
			  	int app= atomic_add_return(1,&(memory->pending_migration));
                        	if(app==migrating_threads)
                        		atomic_set(&(memory->pending_migration),(2*migrating_threads));
			}
			else
...
res = create_proc_entry("number_migrating_threads", S_IRUGO, NULL);
if (!res) {
